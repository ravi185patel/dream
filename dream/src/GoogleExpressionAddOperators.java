...
/*
given 3 integer return expression with ) and ( brackets.
 */
public class GoogleExpressionAddOperators {
        ...
        for (int i = start; i < num.length(); i++) {
            ...
            if (start == 0) {
                ...
                dfs(num, target, i + 1, current_value + current_num_val, current_num_val,
                        expression + "+" + current_num + (i+1 >= num.length() ? "":")"), result);

                dfs(num, target, i + 1, current_value - current_num_val, -current_num_val,
                        expression + "-" + current_num + (i+1 >= num.length() ? "":")"), result);

                dfs(num, target, i + 1, current_value - last_operand + last_operand * current_num_val,
                        last_operand * current_num_val, expression + "*" + current_num + (i+1 >= num.length() ? "":")"), result);
            }
        }
    }
}
